...
begin
     if average > winningmean then begin {if}
        winningmean := average;
        champion := contestant; gold;
     end; {if}
end; {gold}

procedure silver; {silver}
begin
     if (average < winningmean) and (second > average) then begin {if}
        second := average;
        number2 := contestant;
     end; {if}
end; {silver}

procedure bronze; {bronze}
begin
     if (average > third) and (average < second) then begin {if}
        third := average;
        number3 := contestant;
     end {if}
end; {bronze}
...
begin {main}
     ...
     winningmean := 3; second := 2; third := 1;
           while not eof do begin {while}
               average := 0;
               numsum;
               readln;
               average := abs(sum - highest - lowest)/(num - 2);
